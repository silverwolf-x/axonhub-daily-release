...
set_result() {
...

  {
    ...
    echo "- Force publish: \`${FORCE_PUBLISH:-false}\`"
  } >> "$GITHUB_STEP_SUMMARY"
}
...
if [[ "$last_upstream_commit" == "$upstream_head" && \
  "$consolidated" == 'true' && "${FORCE_PUBLISH:-false}" != 'true' ]]; then
  echo "Fork '$FORK_BRANCH' is current and already has one customization commit."
  set_result idle
  exit 0
fi

# A manual force_publish run intentionally bypasses the sync-only path below
# and falls through to the RC creation path, even when upstream has no new
# releasable commits.
